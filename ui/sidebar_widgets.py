...
class RangeSlider(QWidget):
    ...
    def sizeHint(self):
        try:
            ...
            return _QSize(200, max(22, self.minimumHeight()))
        except Exception:
    ...
    def paintEvent(self, event):
        ...
        painter.drawRect(rect.left(), cy - track_h // 2, rect.width(), track_h)
        ...
        painter.drawRect(min(x1, x2), cy - track_h // 2, abs(x2 - x1), track_h)
        # Handles
        handle_w = max(8, track_h * 2)
        painter.setBrush(QColor('#dddddd'))
        painter.setPen(QPen(QColor('#222222'), 1))
        painter.drawRect(x1 - handle_w // 2, cy - track_h, handle_w, track_h * 2)
        painter.drawRect(x2 - handle_w // 2, cy - track_h, handle_w, track_h * 2)
    ...
    def mousePressEvent(self, event):
        ...
        click_padding = 4
        low_rect = (x1 - handle_w // 2 - click_padding, cy - track_h - click_padding, handle_w + 2 * click_padding, track_h * 2 + 2 * click_padding)
        high_rect = (x2 - handle_w // 2 - click_padding, cy - track_h - click_padding, handle_w + 2 * click_padding, track_h * 2 + 2 * click_padding)
        if (low_rect[0] <= x <= low_rect[0] + low_rect[2]) and (low_rect[1] <= y <= low_rect[1] + low_rect[3]):
            self._dragging = 'low'
            self._setValuesInternal(self._pos_to_value(x), self._high)
        elif (high_rect[0] <= x <= high_rect[0] + high_rect[2]) and (high_rect[1] <= y <= high_rect[1] + high_rect[3]):
            self._dragging = 'high'
